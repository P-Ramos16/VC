...
# Read the image
colourImage = cv2.imread("../images/deti.bmp", cv2.IMREAD_UNCHANGED)
...
row1_image = cv2.hconcat([colourImage, greyscaleImage])
row2_image = cv2.hconcat([hslImage, hsvImage])
row3_image = cv2.hconcat([xyzImage, blackImage])

final_image = cv2.vconcat([row1_image, row2_image, row3_image])
